...
model_choice = st.sidebar.selectbox(
    "Choose a Model", 
    ("Logistic Regression", "Random Forest", "SVM", "K-Nearest Neighbors")
)
pca_components = st.sidebar.slider(
    "Number of PCA Components", min_value=2, max_value=X.shape[1], value=10
)
...
if pca_components >= 2:
    ...
    for target_value, target_name in enumerate(target_names):
        ...
        ax_scatter.scatter(subset[:,0], subset[:,1], label=target_name, alpha=0.7)
    ax_scatter.set_xlabel("PC1")
    ax_scatter.set_ylabel("PC2")
    ax_scatter.set_title("Breast Cancer PCA Scatter Plot")
    ax_scatter.legend()
